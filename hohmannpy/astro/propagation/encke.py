...
class EnckePropagator(base.Propagator, universal_variable.UniversalVariablePropagator, cowell.CowellPropagator):
    def __init__(
            self,
            step_size: float = 60,
        ...
        base.Propagator.__init__(self, step_size)
    ...
    def step(self, name, satellite, time_change):
        ...
        del_state = cowell.CowellPropagator.rk4(
            self,
            t0=orbit.time,
            delt=time_change,
        ...
        ref_position = f_func * self.initial_positions[name] + g_func * self.initial_velocities[name]
        fdot_func = (
                np.sqrt(orbit.grav_param)
                / (np.linalg.norm(ref_position) * np.linalg.norm(self.initial_positions[name]))
                * orbit.universal_variable * (orbit.stumpff_param * s_func - 1)
        )
        if self.fg_constraint:
            gdot_func = (g_func * fdot_func + 1) / f_func
        else:
            gdot_func = 1 - orbit.universal_variable ** 2 / np.linalg.norm(ref_position) * c_func
        ref_velocity = fdot_func * self.initial_positions[name] + gdot_func * self.initial_velocities[name]

        if np.linalg.norm(del_position) / np.linalg.norm(position) > 0.01
        orbit.time += time_change
        orbit.position = ref_position + del_position
        orbit.velocity = ref_velocity + del_velocity
    ...
    def eom(
        ...
        del_y0_dot = y[3] - y_ref[3]
        del_y1_dot = y[4] - y_ref[4]
        del_y2_dot = y[5] - y_ref[5]

        del_y3_dot = -satellite.orbit.grav_param / ref_radius ** 3 * y[0] * (encke_func * encke_param * y[0] - del_y[0])
        del_y4_dot = -satellite.orbit.grav_param / ref_radius ** 3 * y[1] * (encke_func * encke_param * y[1] - del_y[1])
        del_y5_dot = -satellite.orbit.grav_param / ref_radius ** 3 * y[2] * (encke_func * encke_param * y[2] - del_y[2])
